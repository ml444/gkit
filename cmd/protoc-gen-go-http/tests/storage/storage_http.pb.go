...
import (
	...
	response "github.com/ml444/gkit/middleware/response"
...
var _ = new(httpx.Server)
var _ = pluck.DisablePluckHeader

var _ = response.MarkHttpRaw
...
func RegisterStorageHTTPServer(s *httpx.Server, srv StorageHTTPServer) {
	RegisterStorageHTTPServerWithPrefix(s, "", srv)
}

func RegisterStorageHTTPServerWithPrefix(s *httpx.Server, prefix string, srv StorageHTTPServer) {
	r := s.GetRouter()
	r.POST(httpx.JoinPath(prefix, "/storage/upload/v0"), Storage_UploadV00_HTTP_Handler(srv, s.Middlewares()...))
	r.POST(httpx.JoinPath(prefix, "/storage/upload/v1"), Storage_UploadV10_HTTP_Handler(srv, s.Middlewares()...))
	r.POST(httpx.JoinPath(prefix, "/storage/upload/v2"), Storage_UploadV20_HTTP_Handler(srv, s.Middlewares()...))
	r.POST(httpx.JoinPath(prefix, "/storage/download/v0"), Storage_DownloadV00_HTTP_Handler(srv, s.Middlewares()...))
	r.POST(httpx.JoinPath(prefix, "/storage/download/v1"), Storage_DownloadV10_HTTP_Handler(srv, s.Middlewares()...))
	r.POST(httpx.JoinPath(prefix, "/storage/download/v2"), Storage_DownloadV20_HTTP_Handler(srv, s.Middlewares()...))
}
...
func Storage_UploadV10_HTTP_Handler(srv StorageHTTPServer, mws ...middleware.Middleware) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err := pluck.ExtractHeader(r.Header, &in.FileInfo); err != nil {
			ctx.ReturnError(err)
			return
		}
		handler := middleware.Chain(mws...)(func(ctx context.Context, req interface{}) (interface{}, error) {
...
func Storage_DownloadV10_HTTP_Handler(srv StorageHTTPServer, mws ...middleware.Middleware) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		response.MarkHttpRaw(w)
		rsp := out.(*DownloadRsp)
		if err := pluck.SetResponseHeaders(ctx.Response(), rsp.Headers); err != nil {
			ctx.ReturnError(err)
			return
		}
		contentType := ""
		...
		if err := ctx.Blob(http.StatusOK, contentType, rsp.Data); err != nil {
			ctx.ReturnError(err)
			return
		}
	}
