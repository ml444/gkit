...
const StrTpl = `
	...
	{{ else if $r.GetEmail }}
		if err := _validateEmail({{ accessor . }}); err != nil {
			err = {{ errCause .Field "err" "value must be a valid email address" }}
			if !all { return err }
			errors = append(errors, err)
		}
	{{ else if $r.GetHostname }}
		if err := _validateHostname({{ accessor . }}); err != nil {
	...
	{{ else if $r.GetUuid }}
		if err := _validateUuid({{ accessor . }}); err != nil {
	...
	{{ if $r.Pattern }}
		if !{{ lookup .Field "Pattern" }}.MatchString({{ accessor . }}) {
