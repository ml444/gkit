...
import (
	...
	"github.com/ml444/gkit/internal/conn"
	"github.com/ml444/gkit/log"
	"github.com/ml444/gkit/sd"
	"github.com/ml444/gkit/sd/internal/instance"
)
...
func NewInstancer(client Client, logger log.Logger, service string, tags []string, passingOnly bool) *Instancer {
	...
	if err == nil {
		s.logger.Info("instances:", len(instances))
	} else {
...
func (s *Instancer) loop(lastIndex uint64) {
	...
	for {
		...
		case index == defaultIndex:
			s.logger.Info("err:", "index is not sane")
			time.Sleep(d)
			d = conn.Exponential(d)
		case index < lastIndex:
			s.logger.Info("err:", "index is less than previous; resetting to default")
