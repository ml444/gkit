...
- [**protoc-gen-go-http**](https://github.com/ml444/gkit/tree/master/cmd/protoc-gen-go-http): 生成 HTTP 路由与 handler（详见 [插件 README](cmd/protoc-gen-go-http/README.md)）。通过 `pluck.proto` 扩展支持上传/下载。默认 `omitempty=true` 时，没有 `google.api.http` 注解的 proto 不会生成 `*_http.pb.go`。
...
- 注册错误码和错误信息
- 封装错误码、错误信息、错误详情与 cause 链（errors.Unwrap）
...
可以通过在proto文件中定义错误码和错误信息，然后通过 errorx.RegisterError() 注册错误码和错误信息，
...
func (s *UserService) GetUser(ctx context.Context, req *user.GetUserReq) (*user.GetUserRsp, error) {
	if req.Id == 0 {
        ...
        // 返回英文的错误信息: {"status": 400, "code": 102001, "message": "Missing parameters"}
        // 如果存在请求头: `Accept-Language: zh` 
        // 返回中文的错误信息: {"status": 400, "code": 102001, "message": "缺失参数"}
	}
	// do something
    ...
    // 如果存在请求头: `Accept-Language: en` 
    // 返回英文的错误信息: {"status": 404, "code": 102002, "message": "The user was not found"}
    // 如果存在请求头: `Accept-Language: zh` 
    // 返回中文的错误信息: {"status": 404, "code": 102002, "message": "未找到用户"}
}
...
**Service 中间件**：`response`（错误/空响应/包装）、`recovery`、`tracing`、`requestid`、`ratelimit`、`auth`、`validate`、`logging`、`circuitbreaker`、`timeout`、`bodylimit`、`metrics`、`retry`、`idempotency`。

**HTTP 中间件**：`cors`、`security`、`gzip`、`logging`（含 HTTP 访问日志）、`csrf`、`ipfilter`、`response.WrapHttpResponse`。

**推荐顺序**

- Service：`recovery` → `tracing` → `requestid` → `ratelimit` → `auth` → `validate` → 业务 → `logging` → `response.WrapError` → `response.WrapResponse`
...
分布式限流：可选子模块 `middleware/ratelimit/redis`，key 格式 `gkit:rl:{service}:{path}:{windowMs}`。见 [middleware/OPTIONAL.md](middleware/OPTIONAL.md)。
Prometheus：`-tags prometheus` 编译。OTel：`middleware/tracing/otel`。CSRF 默认跳过 Bearer API。
...
`grpcx` 封装了 `grpc.Server` / `grpc.ClientConn`，提供 transport 上下文、中间件、健康检查，并通过 [`discovery`](discovery) 模块支持**服务发现**。

**gkit 注册中心（Consul、Etcd、Nacos 等）** — 与 httpx 相同的 `discovery:///` 地址格式：
...
func main() {
	...
	defer client.Close()
	conn := client.Conn() // 配合 protoc 生成的 stub 使用
	_ = conn
}
```

**Istio / Envoy mesh（xDS）** — 独立控制平面，使用 `grpcx/xds` 子包（需配置 `GRPC_XDS_BOOTSTRAP`）：
...
同一 `ClientConn` 不要混用 `discovery:///` 与 `xds:///`；每个连接只选一种发现方式。
